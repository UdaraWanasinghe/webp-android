...
JNIEXPORT jlong
JNICALL
        Java_com_aureusapps_webpcodec_encoder_WebPAnimEncoder_create(JNIEnv * env, jobject
self,
jint width, jint
height,
jobject options
...
jobject bitmap = env->GetObjectField(frame, env->GetFieldID(frame_class, "bitmap",
...
encoder = encoder;
user_data->
frame_id = ++encoder->frame_count;
webp_picture.
user_data = user_data;
webp_picture.
progress_hook = Encoder::OnProgressUpdate;
if (
WebPAnimEncoderAdd(encoder
->anim_encoder, &webp_picture, timestamp, &encoder->encoder_config)) {
} else {
ThrowException(env,
...
if (
WebPAnimEncoderAdd(encoder
->anim_encoder, nullptr, timestamp, nullptr)) {
WebPData data;
WebPDataInit(&data);
if (
WebPAnimEncoderAssemble(encoder
->anim_encoder, &data)) {
...
->
GetFieldID(encoder_class,
"nativeObjectPointer", "J"), (jlong) 0);
env->
DeleteWeakGlobalRef(encoder
->java_encoder_ref);
delete
encoder;
} else {
